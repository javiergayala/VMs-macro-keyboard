...
//
// +---------+--------+-----------+----------+
// | PIN7    | PIN6   | PIN14     | PIN15    |
// | Zoom    | Slack  | Msgs      | iTerm    |
// +---------+--------+-----------+----------+
// | PIN9    | PIN8   | PIN10     | PIN16    |
// | Firefox | Safari | ScreenSvr | Modifier |
// +---------+--------+-----------+----------+
...
void setup() {
  ...
  encoder = new ClickEncoder(ENCODER_DT, ENCODER_CLK, ENCODER_SW, STEPS_PER_NOTCH); // Initializes the rotary encoder with the mentioned pins
  ...
  // Turn off the built-in TX/RX LEDs.  The blinking was driving me nuts.
  ...
  if (b != ClickEncoder::Open) {                 // If the button is unpressed, we'll skip to the end of this if block
    switch (b) {
      case ClickEncoder::Clicked:                // Button was clicked once
        Consumer.write(MEDIA_VOLUME_MUTE);        // Replace this line to have a different function when clicking button once
